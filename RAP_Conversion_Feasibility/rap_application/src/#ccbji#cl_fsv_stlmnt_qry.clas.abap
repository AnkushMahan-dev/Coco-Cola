...
CLASS /ccbji/cl_fsv_stlmnt_qry IMPLEMENTATION.
  ...
  METHOD if_rap_query_provider~select.
    ...
    TRY.
        ...
        " Same idea as the blank TOUR path, but the tours are restricted to the
        " entered Shipment / Visit List. Builds ONLY the requested page's tours
        " and gets the total from a cheap DB COUNT, so a wide range no longer
        " rebuilds the whole result on every scroll. Output is IDENTICAL to the
        " slow path (same tours, same count, same read_tour values); only the row
        " order becomes deterministic (tour_id). Engages ONLY when the sole key
        " is the Visit List (no plant/route/date/status), no detail/driver/
        " vehicle/tpp filter, no custom sort, not a by-key read - every other
        " case falls through UNCHANGED to the proven slow path below.
        IF lv_mode = 'TOUR'
          ...
          " get_tours does (interval passthrough + raw / zero-padded / stripped
          " variants for a single value), so the tour set is identical.
          ...
          " Matching tour ids from the entered Visit List range (same set the
          " slow path's get_tours resolves). Materialised into a RANGE - the
          " existing code uses this to bridge the /DSD/ST_STATUS-TOURID vs
          " /dsd/hh_tour_id type difference (avoids a subquery type mismatch).
          ...
          IF lr_ftid IS INITIAL.
            " No tours match - empty page, count 0 (no wrong data, no dump).
            io_response->set_data( VALUE tt_result( ) ).
            IF io_request->is_total_numb_of_rec_requested( ).
              io_response->set_total_number_of_records( 0 ).
            ENDIF.
          ...
          " Build ONLY the page's tours (same builder as the slow path).
          DATA lt_fptour TYPE tt_tour.
          IF lt_fptid IS NOT INITIAL.
            DATA lr_fppage TYPE RANGE OF /dsd/hh_tour_id.
            CLEAR lr_fppage.
            LOOP AT lt_fptid ASSIGNING FIELD-SYMBOL(<fpt>).
              APPEND VALUE #( sign = 'I' option = 'EQ' low = <fpt>-tour_id ) TO lr_fppage.
            ENDLOOP.
